...
public class OrderApprovalSaga implements SagaStep<RestaurantApprovalResponse> {
    ...
    public void rollback(RestaurantApprovalResponse restaurantApprovalResponse) {
        ...
        // for order-rejected event (via restaurent service - orderrejected publisher):
        //order will be in cancelling state and the saga status will be in compensating state
        SagaStatus sagaStatus = orderSagaHelper.orderStatusToSagaStatus(domainEvent.getOrder().getOrderStatus());

        approvalOutboxHelper.save(getUpdatedApprovalOutboxMessage(orderApprovalOutboxMessage,
                domainEvent.getOrder().getOrderStatus(), sagaStatus));// update approvalorderoutbox saga status from processing to compensating
