...
public class CustomerMessageKafkaListener implements KafkaConsumer<CustomerAvroModel> {
    ...
    public CustomerMessageKafkaListener(CustomerMessageListener customerMessageListener, OrderMessagingDataMapper orderMessagingDataMapper) {
    ...
    public void receive(@Payload List<CustomerAvroModel> messages,
        ...
        messages.forEach(customerAvroModel -> {
            log.info("Initialising write sequence for customer id:{}",customerAvroModel.getId());
            CustomerRequest request = orderMessagingDataMapper.customerAvroModelToCustomerRequest(customerAvroModel);

            customerMessageListener.persistCustomer(request);
        });
